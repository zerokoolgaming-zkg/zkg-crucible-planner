// ====== CONFIG ======
const API_URL = "const API_URL = "https://script.google.com/macros/s/AKfycbwgqUa14K4cMZVDP7qxg6rqDs17vGKSiWCVJOavIA-D0oC6o2SkhvqwJHuJ4oZdqtE/exec";
";
...
const rangeA1El = document.getElementById("rangeA1");
const valuesJsonEl = document.getElementById("valuesJson");
const updateBtn = document.getElementById("updateBtn");
const updateMsg = document.getElementById("updateMsg");

let token = null; // for this simple demo we reuse the password per-request

loginBtn.addEventListener("click", async ()=>{
  const pw = pwdEl.value.trim();
  loginMsg.textContent = "";
  try{
    const u = `${API_URL}?action=admin_get_sheet_snapshot&password=${encodeURIComponent(pw)}`;
