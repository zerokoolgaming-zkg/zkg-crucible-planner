...
async function loadCharacters(){
  try{const r=await fetch(`${API_URL}?action=getcharacters`);
    ...
    characters=d.characters||[];return true;
...
async function onSearch(){
  ...
  if(team.length!==5){resultEl.innerHTML=`<div class="muted">Please select exactly 5 characters.</div>`;return;}
  lockUI(true);
  ...
  }catch(e){resultEl.innerHTML=`<div class="muted">Error: ${e.message}</div>`;}
  ...
  const cards = matches.map((m,i)=>{
    ...
    return `
      <div class="match-card">
        <h3>Match #${i+1}</h3>
        <div class="grid5">${imgRow}</div>
        <div class="info">
          <p><b>Room:</b> ${m.room || '-'}</p>
          <p><b>Punch-up / Down:</b> ${m.punchup || '-'}</p>
          <p><b>TCP Diff:</b> ${m.tcpDiff || '-'}</p>
          <p><b>Combo / Notes:</b> ${m.combo || '-'}</p>
        </div>
      </div>`;
  }).join('');
