/* ZKG Planner v4.5-root (live indicator + auto re-ping) */
const BACKEND_URL = "https://script.google.com/macros/s/AKfycbweQfrrV7MW5Z49MxYWe6nQ25Y5whGe_VHXZ6-gkR8x/exec";
const FALLBACK = {
...
const REPING_MS = (window.LIVE_REPING_MS|| (5*60*1000));
...
init();
setInterval(checkLive, REPING_MS);
...
async function checkLive(){
  try { await safeJson(`${BACKEND_URL}?action=ping`); setLive(true); }
  catch(e) { setLive(false); }
}

function setLive(ok){
  if(ok){ liveDot.classList.remove('red'); liveDot.classList.add('green'); liveText.textContent='Live'; }
  else  { liveDot.classList.remove('green'); liveDot.classList.add('red'); liveText.textContent='Offline'; }
}

async function loadAll(){
  try {
    await safeJson(`${BACKEND_URL}?action=ping`);
...
async function onSearch(){
  ...
  try{
    const r = await safeJson(`${BACKEND_URL}?action=findcounters&team=${encodeURIComponent(JSON.stringify(team))}`);
