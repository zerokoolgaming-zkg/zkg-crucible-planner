/* ZKG v4.1.4-fix1 */
// === BACKEND CONNECTION ===
const BACKEND_URL = "https://script.google.com/macros/s/AKfycbyJ6teF8hhxV1dO_26kFSF8uEuVR00gTN9eEe9PLmYl3Oc-pjQbU6REctlPu68Rly0XGw/exec";
...
async function loadAll(){
  const [chars, tms, pngs] = await Promise.all([
    safeJson(`${API_URL}?action=getcharacters`),
    safeJson(`${API_URL}?action=getteams`),
    safeJson(`${API_URL}?action=getpngmap`)
  ]);
  characters = chars.characters || [];
  teams      = (tms.teams || []).map(t=> t.team ? {name:t.team, members:t.members||[]} : t );
...
async function onSearch(){
  ...
  try{
    const r = await safeJson(`${API_URL}?action=findcounters&team=${encodeURIComponent(JSON.stringify(team))}`);
